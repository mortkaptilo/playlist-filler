...
export function Recommendations() {
  ...
  const onClickRecommendation = (track: Track) => {
    setSeedTracks((prev) => {
      fillPlaylist([...prev, track]);
      return [...prev, track];
    });
  };

  async function fillPlaylist(tracks: Track[]) {
    try {
      const recs = await getRecommendations(tracks.map((t) => t.track_id));
    ...
    <ul className="flex flex-col gap-1 max-h-[calc(100vh-260px)] overflow-y-auto pr-1">
      {recommendations.map((track, i) => (
        ...
        >
          ...
          <button
            onClick={() => {
              onClickRecommendation(track);
            }}
            className="opacity-90 group-hover:opacity-100 mr-2 shrink-0 transition-opacity text-zinc-500 hover:text-green-400 p-1 rounded-md hover:bg-green-400/10
                    cursor-pointer"
          >
